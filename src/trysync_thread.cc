...
void TrysyncThread::PrepareRsync() {
  ...
  if (!g_conf.db_name.empty()) {
    if (db_sync_path.back() != '/') {
      db_sync_path.append("/");
    }
    pstd::CreatePath(db_sync_path + g_conf.db_name);
  }
}
...
bool TrysyncThread::TryUpdateMasterOffset() {
  ...
  if (!pstd::FileExists(info_path)) {
    std::string alt_path = db_sync_path;
    if (!alt_path.empty() && alt_path.back() != '/') {
      alt_path.append("/");
    }
    alt_path.append(g_conf.db_name);
    alt_path.append("/");
    alt_path.append(kBgsaveInfoFile);
    if (!pstd::FileExists(alt_path)) {
      return false;
    }
    info_path = alt_path;
  }
